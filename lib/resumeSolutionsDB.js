...
const {
    ...
    addToAllStepBoards, getStepBoardsForAllSteps, updateStepBoard, countStepBoardsForAllSteps, getStepBoardsForAllSteps_CURSOR,
} = require("./saveToDb");
...
module.exports.findSymmetricBoards = async function (endCb) {
    ...
    for (const stepToCompare of steps) {
        const sameStepsByBoardString = await getStepBoardsForAllSteps({ boardString: stepToCompare.boardString, _id: { $ne: stepToCompare._id } });
        console.log('sameStepsByBoardString', sameStepsByBoardString.length);
        const stepsToCompareCursor = await getStepBoardsForAllSteps_CURSOR({
            blanks: lastRunningStep.blanks,
            _id: { $lt: stepToCompare._id },
            aliasStatus: { $in: ['NOT_SEARCHED', 'NONE_FOUND'] }, // all unique -- skipping already duplicates
        });
        let foundStep;
