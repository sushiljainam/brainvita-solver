...
const { getProgress, addToStepBoards, addToProgress, getStepBoards,
    addToAllStepBoards, getStepBoardsForAllSteps } = require("./saveToDb");
...
async function findBoardsForAllSteps(blanks) {
    ...
    const savedSteps = await getStepBoardsForAllSteps({ blanks });
...
/**
 * [x] fetch progress coll
 * [_x_] get last RUNNING/undone step number
 * [ ] fetch saved boards from allsteps coll for step number
 * [ ] with (movesPicked, aliasStatus)
 */
module.exports.saveAllStepsBoards = async function (endCb) {
    ...
    // const nextBoards = reduce(async (nextBoardsYet, { board, movesPicked, _id }) => {
    let nextBoards = [];
